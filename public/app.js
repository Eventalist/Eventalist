...
$("button#subscribeUser").on("click", function(){
	...
	$.post("http://localhost:5000/users", {name: name, email: email}, function(user){		

		if ($("input.art").prop("checked") == true){
			$.post("http://localhost:5000/subscriptions", {user_id: user.id,name: name, email: email, category_id: 1})
		};
		if ($("input.music").prop("checked") == true){
			$.post("http://localhost:5000/subscriptions", {user_id: user.id,name: name, email: email, category_id: 2})
		};
		if ($("input.theater").prop("checked") == true){
			$.post("http://localhost:5000/subscriptions", {user_id: user.id,name: name, email: email, category_id: 3})
		};

		if ($("input.free").prop("checked") == true){
			$.post("http://localhost:5000/subscriptions", {user_id: user.id,name: name, email: email, category_id: 4})
		};
		...
		var sendEmail = function(){
			$.get('http://localhost:5000/users/' + user.id + '/subscriptions')
		}
